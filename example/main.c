...
static flash_ospi_context_t host;
...
int main(void)
{
    ...
    const flash_config_t config = {
        ...
        .host = flash_ospi_bind(&host, &hospi1,
                                __HAL_RCC_GET_OSPI_SOURCE() == RCC_OSPICLKSOURCE_HCLK ? HAL_RCC_GetHCLKFreq()
                                                                                      : 0U),
    ...
    if (example_result == STM_OK)
    {
        example_result = flash_get_info(device, &info);
    }
...
static void board_init(void)
{
    ...
    if (HAL_OSPIM_Config(&hospi1, &manager, 100U) != HAL_OK)
    {
        Error_Handler();
    }
    HAL_Delay(10U);
